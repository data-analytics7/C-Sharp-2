...
{
    //Soure: https://weblogs.asp.net/jimjackson/using-linq-to-xml-with-c-to-read-gpx-files
    //Class used for parsing GPX files
    class GPXManager
    {     
        // <summary>
        ...
        public string LoadGPXWaypoints(string sFile)
        {
            ...
            {
                // This is where we'd instantiate data containers for the information retrieved.
                sb.Append(
        ...
        public List<PointLatLng> LoadGPXTracks(string sFile)
        {
            ...
            foreach (var trk in tracks)
            {
                ...
                foreach (var trkSeg in trk.Segs)
                {
                    ...
                    sb.Append(
                      string.Format("Latitude:{1} Longitude:{2}\n",
                      trk.Name, trkSeg.Latitude,
                      trkSeg.Longitude, trkSeg.Elevation,
                      trkSeg.Time));

                    PointLatLng point = new PointLatLng();
                    point.Lng = Convert.ToDouble(trkSeg.Longitude);
                    point.Lat = Convert.ToDouble(trkSeg.Latitude);
                    trackList.Add(point);
                }
            }
            return trackList;
